...
def classification_page(tab_selection=None):
    ...
    # Onglet Traitement
    if tab_selection == "Traitement des données":
        st.subheader("Traitement des données")
        process_data_section()
    
    # Onglet Visualisation
    elif tab_selection == "Visualisation":
        st.subheader("Visualisation des données")
        visualization_section()
    
    # Onglet Entraînement & Évaluation
    elif tab_selection == "Entraînement et Evaluation":
        st.subheader("Entraînement et Évaluation du modèle")
        training_section()
    
    # Si aucun onglet n'est sélectionné, afficher tous les onglets
    else:
        tabs = st.tabs(["Traitement", "Visualisation", "Entraînement et Evaluation"])
        with tabs[0]:
            process_data_section()
        with tabs[1]:
            visualization_section()
...
def process_data_section():
    ...
    file_path = st.text_input("Chemin vers le fichier CSV :", "cleaned.csv")
...
def visualization_section():
    if 'X_selected' in st.session_state and 'y' in st.session_state:
        ...
        st.warning("Veuillez d'abord traiter les données dans l'onglet 'Traitement des données'")
