...
def regression_page():
    ...
    st.caption(
        "Analyse exploratoire, sélection des variables et évaluation d'un modèle de régression"
    )
    ...
    target = st.selectbox(
        "Sélectionnez la colonne cible pour la régression :", data.columns
    )
    ...
    for name, model in models.items():
        ...
        results.append(
            {"Modèle": name, "R²": r2, "MSE": mse, "Erreur Médiane": med_error}
        )
